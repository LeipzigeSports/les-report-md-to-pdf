...
func handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		cmd := exec.CommandContext(cmdCtx, cfg.pandocExecutable, tmpIn.Name(), "-f", "commonmark+pipe_tables", "-o", tmpOut.Name(), "-t", "pdf", "--template", cfg.pandocTypstTemplatePath, "-V", fmt.Sprintf("team=%v", teamName), "--pdf-engine", "typst", "--pdf-engine-opt", "--pdf-standard=a-2b")
